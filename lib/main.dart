...
class Secondappstate extends State<MyApp> {
  ...
  String selectedDeviceType = 'Select Device Type';
            ...
            Future.delayed(Duration(milliseconds: 300), () {
              showDialog(
                ...
                builder: (_) => AlertDialog(
                  title: Text("Wipe Completed"),
                  content: Text(
                    "Your disk has been securely wiped and reformatted (NTFS/FAT32).",
                    style: TextStyle(fontSize: 16),
                  ),
                  actions: [
                    TextButton(
...
String _extractDeviceFromListLine(String line) {
  ...
  Future<void> startDoD522022MWipe() async {
    ...
    if (selectedDeviceType == 'Select Device Type') {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Select a device type')));
      return;
    }
              ...
              content: Column(
                ...
                children: [
                  ...
                  Text("Disk: $selectedDisk",
                      style: TextStyle(fontWeight: FontWeight.bold)),
                  SizedBox(height: 10),
                  Text("Device Type: $selectedDeviceType",
                      style: TextStyle(fontWeight: FontWeight.bold)),
                  SizedBox(height: 10),
                  Text(
                    "THIS WILL PERMANENTLY DESTROY ALL DATA!\nDisk will be formatted (NTFS/FAT32) after wiping.",
                    style: TextStyle(color: Colors.red, fontWeight: FontWeight.bold),
                    textAlign: TextAlign.center,
                  ),
                ],
...
This app securely erases data using DoD 5220.22-M standard, NIST CLEAR, Gutmann (simplified), or Single Pass.
- Passes depend on the chosen method (DoD: 3 passes; NIST: 1 pass + metadata wipe; Gutmann: simplified 4 passes)
- Final step: Create new partition and format (NTFS/FAT32)
Disk remains usable after wiping.''',
                style: TextStyle(
                    color: Colors.white,
                    fontSize: 15,
          ...
          Column(
            children: [
              ...
              // NEW DEVICE TYPE DROPDOWN (Same Style)
              Container(
                alignment: Alignment.center,
                decoration: BoxDecoration(
                    color: Colors.grey,
